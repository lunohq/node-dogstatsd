...
function merge_tags(global_tags, tags) {
  var merged_tags = [];
  if (Array.isArray(global_tags))
    merged_tags = merged_tags.concat(global_tags);
...
Client.prototype.check = function(name, status, meta) {
  var self = this;
  var check = ["_sc", name, status];
  var metadata = meta || {};
  // The service check message must always be last
  if (metadata["timestamp"]) {
    check.push("d:" + metadata["timestamp"]);
  }
  if (metadata["hostname"]) {
    check.push("h:" + metadata["hostname"]);
  }
  if (this.global_tags || metadata["tags"]) {
    var merged_tags = merge_tags(this.global_tags, metadata["tags"]);
    if (merged_tags.length > 0) {
      check.push("#" + merged_tags.join(','));
    }
  }
  if (metadata["message"]) {
    check.push("m:" + metadata["message"]);
  }
  var send_data = check.join("|");
...
Client.prototype.send = function(data, sample_rate, tags) {
    ...
    if (this.global_tags || tags) {
      var merged_tags = merge_tags(this.global_tags, tags)
